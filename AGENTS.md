...
- タイトルは [Conventional Commits](https://www.conventionalcommits.org/) 互換の書式で記載（例: `fix: auto-adjust layouts to fit a4 output`、`docs: update agents guide for pr process`）
- 説明文は次のテンプレートを基本とし、見出しを付けて情報を整理
  - `## 背景`: 課題・要望・バグ報告など、変更が必要になった理由を明記
  - `## 対応内容`: 主要なロジック・設定・依存追加等を箇条書きで整理（影響範囲やリスクも記載）
  - `## テスト`: 実行したコマンド（例: `npm run lint`, `npm run test:layout`, pre-commit フックなど）と結果を列挙
  - 必要に応じて `## 補足` / `## スクリーンショット` / `## 既知の課題` などの節を追加し、レビュアーが再現・検証する際の注意点や確認方法を共有
- **PR の Description は日本語で記述し、レビュアーが作業の意図と影響範囲を把握できるよう丁寧にまとめること**
  - 変更の背景（課題・要望・バグ情報など）
  - 行った対応内容と主要なファイルやロジックのポイント
  - 実行したテスト・確認手順（コマンド出力やスクリーンショットを含む）
  - レビュアーが再現や検証を行う際の補足情報（必要な環境設定など）
- 画面変更がある場合はスクリーンショットを添付
- 依存追加や設定変更を含む場合はレビュアーに明示し承認を得る
- 推奨ベストプラクティス
  - 関連 Issue やタスクへのリンク（`Closes #123` など）を明記し、追跡性を確保
  - 変更前後の挙動が分かる差分説明（Before/After、GIF、ログ）を添付
  - ブレイキングチェンジやリスクを箇条書きで整理し、ロールバック方法があれば併記
  - レビュアーに見てほしい重点箇所（設計選択、トレードオフ、レビュー観点）を明示
  - 追加レビューが必要な専門領域（例: アクセシビリティ、印刷品質、CI 設定）には担当 reviewer もしくはチームを指定
  - 長期的なフォローアップが必要なTODO/非解決点は `## 既知の課題` やチェックリストで管理
